...
file="786_830_disease_null_PCA_v2.csv"
...
LRT=read.table('nd_t.value.pLRT.value.csv', header=TRUE, sep=',', stringsAsFactors = FALSE)
rownames(LRT)=LRT$X
...
p.phase=data.vol[,c(1,3,5,8)]#prolif phase
s.phase=data.vol[,c(2,4,6,7)]#sec phase

p.avg=data.frame(rowMeans(p.phase)) #average of log2 fold change
s.avg=data.frame(rowMeans(s.phase))#average of log2 fold change

log2fd=data.frame(p.avg-s.avg)
...
EnhancedVolcano(toptable=lrt.subset,
                ...
                x = 'Log2FoldChange',
                y = 'LRT.pvalue',
                title= "Secretory Vs Proliferation",
                pCutoff = 0.05,
                FCcutoff = 0.5,
                labSize = 4.0)
